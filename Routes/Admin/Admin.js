...
Only return the JSON with proper latex for maths. Ensure LaTeX syntax is clean and unescaped. Do not include any extra commentary or markdown. Also remove all \n and \\ . Modify the data to display on the website using mathJx. Please do not skip the latex syntax for maths and depict fraction in the syntax $\frac{2}{5}$ and before returning the extracted the text, please recheck that each latex produced is correct to will be rendered perfectly. Always enclose LaTeX with $ like this $\frac{2}{5}$ and use only single slashes in the LaTeX. also detect and preserve underlined and italic text and lines, manually wrap the underlined line or text inside <u></u> without using latex and use html and inline css in html tags like in  li , ul , li should have bullets,  always use style="list-style-type: circle; inside ul tag , etc to make text more beautiful to render only in passage text. make sure to not use any html in options and explaination text.`
                },
                {
                    role: 'user',
                    content: [
